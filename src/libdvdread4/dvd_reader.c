...
#ifdef __amigaos4__
	// start with CSS support, as thats what everyone else is doing :-)
  have_css = dvdinput_setup(0);
#else
...
{
	BPTR lock = Lock( path_copy, EXCLUSIVE_LOCK );
	...
	// we need to foce file io, CSS can't be used, on directorys.
	have_css = dvdinput_setup(1);
...
static int findDVDFile( dvd_reader_t *dvd, const char *file, char *filename )
{
...
#else
  /* Strip off the directory for our search */
  if( !strncasecmp( "VIDEO_TS/", file, 9 ) ) {
    nodirfile = &(file[ 9 ]);
  } else {
    nodirfile = file;
  }
  ...
  if( ret < 0 ) {
	...
	if ((dvd->path_root[strlen(dvd->path_root) -1] == ':' )||(dvd->path_root[strlen(dvd->path_root) -1] == '/' ))
	{
		sprintf( video_path, "%sVIDEO_TS/", dvd->path_root );
	}
	else
	{
