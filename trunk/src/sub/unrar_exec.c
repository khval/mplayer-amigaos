...
#else
	printf("%s:%d AmigaOS4, does not have pipe(), but this can be emulated\n",__FUNCTION__,__LINE__);
#endif
...
#else
	printf("%s:%d AmigaOS4, don't have fork(), we need a create a process to do this\n",__FUNCTION__,__LINE__);
	pid =0;
#endif
...
#else
	printf("%s:%d AmigaOS4, don't have waitpid(), but this can be worked out by using Wait(sigs) \n",__FUNCTION__,__LINE__);
	pid =0;
#endif
