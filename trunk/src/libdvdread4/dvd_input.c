...
static int file_read(dvd_input_t dev, void *buffer, int blocks, int flags)
{
  ...
  while(len > 0) {
    ...
    if(ret == 0) {
      ...
      /*int64_t pos =*/ lseek(dev->fd, over_read, SEEK_CUR);
...
#ifdef __amigaos4__
int dvdinput_setup( int foce_file_io )
#else
...
#endif
{
  void *dvdcss_library = NULL;
	...
	printf("when do we see this evil thing?\n");
...
#ifdef __amigaos4__
  if ((dvdcss_library != NULL) && (foce_file_io != 1)) {
#else
  if(dvdcss_library != NULL) {
#endif
    /*
