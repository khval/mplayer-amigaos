...
//Packs are freed.
static float read_first_mpeg_pts_at_position(demuxer_t* demuxer, long long int stream_pos)
{
...
static demuxer_t* demux_mpg_open(demuxer_t* demuxer) {
  ...
  while (demuxer->video->packs + demuxer->audio->packs < 2)
    ...
    )
    {
      ...
      long long int pos = stream_tell(s);
                           ...
                           float audio_delay, int flags)
{
    ...
    int precision = 1;
    float oldpts = 0;
    long long int oldpos = demuxer->filepos;
    float newpts = 0;
    long long int newpos = (flags & SEEK_ABSOLUTE) ? demuxer->movi_start : oldpos;
...
static demuxer_t* demux_mpg_ps_open(demuxer_t* demuxer)
{
    ...
    if(sh_video && !sh_video->format && ps_probe > 0) {
        int head;
        long long int pos = stream_tell(demuxer->stream);
